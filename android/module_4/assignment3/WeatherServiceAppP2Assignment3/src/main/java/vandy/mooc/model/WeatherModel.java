...
public class WeatherModel
    ...
    GenericServiceConnection<WeatherCall> syncConnection;
    GenericServiceConnection<WeatherRequest> asyncConnection;
    ...
    public void onCreate(MVP.RequiredPresenterOps presenter) {
        ...
        syncConnection = new GenericServiceConnection<>(WeatherCall.class);
        asyncConnection = new GenericServiceConnection<>(WeatherRequest.class);
    ...
    private void bindService() {
        ...
        context.bindService(WeatherServiceSync.makeIntent(context), syncConnection, Context.BIND_AUTO_CREATE);
        context.bindService(WeatherServiceAsync.makeIntent(context), asyncConnection, Context.BIND_AUTO_CREATE);
    }
    ...
    private void unbindService() {
        ...
        mPresenter.get().getApplicationContext().unbindService(syncConnection);
        mPresenter.get().getApplicationContext().unbindService(asyncConnection);
    }
    ...
    public boolean getWeatherAsync(String location) {
        // TODO -- you fill in here.
        try {
            asyncConnection.getInterface().getCurrentWeather(location, mWeatherResults);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return mWeatherResults.isBinderAlive();
    }
    ...
    public WeatherData getWeatherSync(String location) {
        // TODO -- you fill in here.
        try {
            List<WeatherData> weatherDatas = syncConnection.getInterface().getCurrentWeather(location);
